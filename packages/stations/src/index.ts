// Israel Railways stations — the single source of truth shared by the mobile app, the website and the server.
// Photos are referenced by key: React Native resolves them via `@better-rail/stations/images`, the web via generated WebP copies.
export type LanguageCode = "he" | "en" | "ar" | "ru"

export interface Station {
  /** Israel Railways station id, e.g. "3700" */
  id: string
  hebrew: string
  english: string
  russian: string
  arabic: string
  lat: number
  lon: number
  /** Photo key — the file name under `images/` without its extension */
  image?: string
  blurhash?: string
  /** Alternative names riders search for (co-names, nicknames) */
  alias?: string[]
}

/** Maps a language code to the station name field. */
export const stationNameField: Record<LanguageCode, keyof Pick<Station, "hebrew" | "english" | "arabic" | "russian">> = {
  he: "hebrew",
  en: "english",
  ar: "arabic",
  ru: "russian",
}

export const stations: Station[] = [
  {
    id: "3700",
    hebrew: "תל אביב - סבידור מרכז",
    english: "Tel Aviv - Savidor Center",
    russian: "Тель-Авив - Мерказ - Центральная",
    arabic: "تل ابيب – ساڤيدور المركز",
    lat: 32.083715,
    lon: 34.798247,
    image: "tlv-center",
  },
  {
    id: "3500",
    hebrew: "הרצליה",
    english: "Herzliya",
    russian: "Герцлия",
    arabic: "هرتسليا",
    lat: 32.163558,
    lon: 34.817406,
    image: "herzeliya",
  },
  {
    id: "3400",
    hebrew: "בית יהושע",
    english: "Bet Yehoshu'a",
    russian: "Бейт-Иегошуа ",
    arabic: "بيت يهوشوع",
    lat: 32.262346,
    lon: 34.860112,
    image: "bet-yehoshua",
  },
  {
    id: "3300",
    hebrew: "נתניה",
    english: "Netanya",
    russian: "Нетания",
    arabic: "نتانيا",
    lat: 32.319915,
    lon: 34.869257,
    image: "netanya",
  },
  {
    id: "3100",
    hebrew: "חדרה - מערב",
    english: "Hadera - West",
    russian: "Хадера - Маарав",
    arabic: "الخضيرة - غرب  ",
    lat: 32.438199,
    lon: 34.899321,
    image: "hadera-west",
  },
  {
    id: "2800",
    hebrew: "בנימינה",
    english: "Binyamina",
    russian: "Биньямина",
    arabic: "بنيامينا",
    lat: 32.514386,
    lon: 34.949573,
    image: "binyamina",
  },
  {
    id: "2820",
    hebrew: "קיסריה - פרדס חנה",
    english: "Caesarea-Pardes Hana",
    russian: "Кейсария - Пардес-Хана",
    arabic: "قيساريا - بارديس حنا",
    lat: 32.485187,
    lon: 34.954099,
    image: "caesarea",
  },
  {
    id: "2500",
    hebrew: "עתלית",
    english: "Atlit",
    russian: "Атлит",
    arabic: "عتليت",
    lat: 32.692924,
    lon: 34.940314,
    image: "atlit",
  },
  {
    id: "2200",
    hebrew: "חיפה - בת גלים",
    english: "Haifa - Bat Galim",
    russian: "Хайфа - Бат-Галим",
    arabic: "حيفا - بات چاليم",
    lat: 32.830498,
    lon: 34.981888,
    image: "bat-galim",
  },
  {
    id: "1300",
    hebrew: "חוצות המפרץ",
    english: "Hutzot HaMifratz",
    russian: "Хоцот ха-Мифрац ",
    arabic: "حوتسوت همفراتس",
    lat: 32.809527,
    lon: 35.054514,
    image: "hotzot-hamifratz",
  },
  {
    id: "700",
    hebrew: "קריית חיים",
    english: "Kiryat Hayim",
    russian: "Кирьят-Хаим",
    arabic: "كريات حاييم",
    lat: 32.824918,
    lon: 35.06429,
    image: "kiryat-haim",
  },
  {
    id: "1400",
    hebrew: "קריית מוצקין",
    english: "Kiryat Motzkin",
    russian: "Кирьят-Моцкин",
    arabic: "كريات موتسكين",
    lat: 32.833133,
    lon: 35.07003,
    image: "kiryat-motzkin",
  },
  {
    id: "1500",
    hebrew: "עכו",
    english: "Ako",
    russian: "Акко ",
    arabic: "عكا",
    lat: 32.928339,
    lon: 35.082984,
    image: "ako",
  },
  {
    id: "2300",
    hebrew: "חיפה - חוף הכרמל",
    english: "Haifa - Hof HaKarmel",
    russian: "Хайфа Хоф ха - Кармель",
    arabic: "حيفا - شاطئ الكرمل",
    lat: 32.7936,
    lon: 34.957404,
    image: "hof-hakarmel",
  },
  {
    id: "8700",
    hebrew: "כפר סבא - נורדאו",
    english: "Kfar Sava - Nordau",
    russian: "Кфар-Саба – Нордау",
    arabic: "كفار سابا - نورداو",
    lat: 32.167369,
    lon: 34.917428,
    image: "kfar-saba",
  },
  {
    id: "1600",
    hebrew: "נהריה",
    english: "Nahariya",
    russian: "Нагария",
    arabic: "نهاريا",
    lat: 33.005075,
    lon: 35.09871,
    image: "nahariya",
  },
  {
    id: "6300",
    hebrew: "בית שמש",
    english: "Bet Shemesh",
    russian: "Бейт Шемеш",
    arabic: "بيت شيمش",
    lat: 31.757821,
    lon: 34.989515,
    image: "beit-shemesh",
  },
  {
    id: "7000",
    hebrew: "קריית גת",
    english: "Kiryat Gat",
    russian: "Кирьят-Гат ",
    arabic: "كريات چات",
    lat: 31.603526,
    lon: 34.777955,
    image: "kiryat-gat",
  },
  {
    id: "5000",
    hebrew: "לוד",
    english: "Lod",
    russian: "Лод",
    arabic: "اللد",
    lat: 31.947694,
    lon: 34.879159,
    image: "lod",
  },
  {
    id: "7300",
    hebrew: "באר שבע - צפון/אוניברסיטה",
    english: "Be'er Sheva - North/University",
    russian: "Беер - Шева Цафон",
    arabic: "بئر السبع - شمال/الجامعة",
    lat: 31.262089,
    lon: 34.809287,
    image: "beer-sheva-university",
  },
  {
    id: "4800",
    hebrew: 'כפר חב"ד',
    english: "Kfar Habad",
    russian: "Кфар ХАБАД",
    arabic: "كفار حباد",
    lat: 31.993188,
    lon: 34.85303,
    image: "kfar-habad",
  },
  {
    id: "4600",
    hebrew: "תל אביב - השלום",
    english: "Tel Aviv - HaShalom",
    russian: "Тель-Авив - ха-Шалом",
    arabic: "تل أبيب - السلام",
    lat: 32.073346,
    lon: 34.793256,
    image: "tlv-hashalom",
  },
  {
    id: "2100",
    hebrew: "חיפה - מרכז השמונה",
    english: "Haifa Center - HaShmona",
    russian: "Хайфа - Мерказ - Центральная",
    arabic: "حيفا المركز - هشمونا",
    lat: 32.822306,
    lon: 34.997177,
    image: "hashmona",
  },
  {
    id: "5010",
    hebrew: "רמלה",
    english: "Ramla",
    russian: "Рамле",
    arabic: "الرملة",
    lat: 31.928809,
    lon: 34.877304,
    image: "ramla",
  },
  {
    id: "8800",
    hebrew: "ראש העין - צפון",
    english: "Rosh Ha'Ayin - North",
    russian: "Рош ха - Айн Цафон",
    arabic: "روش هعاين - شمال",
    lat: 32.120865,
    lon: 34.934527,
    image: "rosh-haayin",
  },
  {
    id: "5300",
    hebrew: "באר יעקב",
    english: "Be'er Ya'akov",
    russian: "Беер-Яаков",
    arabic: "بئير يعكوف",
    lat: 31.932781,
    lon: 34.828591,
    image: "beer-yaakov",
  },
  {
    id: "5200",
    hebrew: "רחובות",
    english: "Rehovot",
    russian: "Реховот",
    arabic: "رحوڤوت",
    lat: 31.908691,
    lon: 34.806351,
    image: "rehovot",
  },
  {
    id: "5410",
    hebrew: "יבנה מזרח",
    english: "Yavne - East",
    russian: "Явне - Восток",
    arabic: "ياڤنه - شرق",
    lat: 31.861702,
    lon: 34.744295,
    image: "yavne-east",
  },
  {
    id: "9100",
    hebrew: "ראשון לציון - הראשונים",
    english: "Rishon LeTsiyon - HaRishonim",
    russian: "Ришон ле-Цион - Ха-Ришоним ",
    arabic: "ريشون لتسيون - هريشونيم",
    lat: 31.948927,
    lon: 34.802622,
  },
  {
    id: "5800",
    hebrew: "אשדוד עד הלום",
    english: "Ashdod - Ad Halom",
    russian: "Ашдод - ад-Халом",
    arabic: "أشدود - عاد هلوم",
    lat: 31.774061,
    lon: 34.666059,
    image: "ashdod",
    alias: ["מטרופול", "Metropol", "Метропол", "متروپول"],
  },
  {
    id: "4250",
    hebrew: "פתח תקווה - סגולה",
    english: "Petah Tikva - Segula",
    russian: "Петах-Тиква - Сгула",
    arabic: "بيتح تكڤا - سچوله",
    lat: 32.111918,
    lon: 34.901335,
    image: "petah-tikva-segula",
  },
  {
    id: "4100",
    hebrew: "בני ברק",
    english: "Bnei Brak",
    russian: "Бней-Брак",
    arabic: "بني براك",
    lat: 32.102938,
    lon: 34.830131,
    image: "bnei-brak",
  },
  {
    id: "3600",
    hebrew: "תל אביב - אוניברסיטה",
    english: "Tel Aviv - University",
    russian: "Тель-Авив - Университет",
    arabic: "تل أبيب - الجامعة",
    lat: 32.103516,
    lon: 34.804499,
    image: "tlv-university",
    alias: ["אקספו", "Expo", "Экспо", "إكسپو"],
  },
  {
    id: "7320",
    hebrew: "באר שבע - מרכז",
    english: "Be'er Sheva - Center",
    russian: "Беер - Шева Мерказ",
    arabic: "بئر السبع - المركز",
    lat: 31.242886,
    lon: 34.798546,
    image: "beer-sheva-center",
  },
  {
    id: "1220",
    hebrew: "מרכזית המפרץ (לב המפרץ)",
    english: "HaMifrats Central Station",
    russian: "Центральная станция Ха-Мифрац",
    arabic: "همفراتس المركزية",
    lat: 32.792942,
    lon: 35.032964,
    image: "hamifrats",
  },
  {
    id: "4900",
    hebrew: "תל אביב - ההגנה",
    english: "Tel Aviv - HaHagana",
    russian: "Тель-Авив - ха-Хагана ",
    arabic: "تل أبيب - ههچناه",
    lat: 32.053978,
    lon: 34.784839,
    image: "tlv-hagana",
  },
  {
    id: "8600",
    hebrew: "נמל תעופה בן גוריון",
    english: "Ben Gurion Airport",
    russian: "Бен-Гурион Аэропорт",
    arabic: "مطار بن چوريون",
    lat: 32.00036,
    lon: 34.870481,
    image: "ben-gurion",
    alias: ["נתבג"],
  },
  // {
  //   id: "6700",
  //   hebrew: "ירושלים - מלחה",
  //   english: "Jerusalem - Malha",
  //   russian: "Иерусалим - Малха",
  //   arabic: "القدس - المالحه",
  //   image: "malha",
  // },
  {
    id: "5900",
    hebrew: "אשקלון",
    english: "Ashkelon",
    russian: "Ашкелон ",
    arabic: "أشكلون",
    lat: 31.676856,
    lon: 34.604447,
    image: "ashkelon",
  },
  {
    id: "7500",
    hebrew: "דימונה",
    english: "Dimona",
    russian: "Димона",
    arabic: "ديمونا",
    lat: 31.068597,
    lon: 35.01171,
    image: "dimona",
  },
  {
    id: "9200",
    hebrew: "הוד השרון - סוקולוב",
    english: "Hod HaSharon - Sokolov",
    russian: "Ход Хашарон - Соколов",
    arabic: "هود هشارون - سوكولوڤ",
    lat: 32.170213,
    lon: 34.901609,
    image: "hod-hasharon-sokolov",
  },
  {
    id: "4170",
    hebrew: "פתח תקווה  - קריית אריה",
    english: "Petah Tikva - Kiryat Arye",
    russian: "Петах Тиква – Кирьят Арье",
    arabic: "بيتح تكڤا - كريات أريه",
    lat: 32.106273,
    lon: 34.863205,
    image: "kiryat-arye",
  },
  {
    id: "5150",
    hebrew: "לוד גני אביב",
    english: "Lod - Gane Aviv",
    russian: "Лод - Ганей Авив",
    arabic: "اللد - چاني أڤيڤ",
    lat: 31.966977,
    lon: 34.87879,
    image: "lod-gane-aviv",
  },
  {
    id: "8550",
    hebrew: "להבים - רהט",
    english: "Lehavim - Rahat",
    russian: "Леавим - Рахат",
    arabic: "لهاڤيم - رهط",
    lat: 31.369907,
    lon: 34.79804,
    image: "lehavim",
  },
  {
    id: "300",
    hebrew: "פאתי מודיעין",
    english: "Pa'ate Modi'in",
    russian: "Патей Модиин",
    arabic: "بأتي موديعين",
    lat: 31.893133,
    lon: 34.960682,
    image: "paate-modiin",
  },
  {
    id: "400",
    hebrew: "מודיעין - מרכז",
    english: "Modi'in - Center",
    russian: "Модиин центр ",
    arabic: "موديعين - المركز",
    lat: 31.901238,
    lon: 35.005712,
    image: "modiin-center",
  },
  {
    id: "4640",
    hebrew: "צומת חולון",
    english: "Holon Junction",
    russian: "Холон - Развязка Холон",
    arabic: "مفترق حولون",
    lat: 32.037036,
    lon: 34.776461,
    image: "holon-junction",
  },
  {
    id: "4660",
    hebrew: "חולון - וולפסון",
    english: "Holon - Wolfson",
    russian: "Холон - Вольфсон",
    arabic: "حولون - ڤولفسون",
    lat: 32.035462,
    lon: 34.759782,
    image: "holon-wolfson",
  },
  {
    id: "4680",
    hebrew: "בת ים - יוספטל",
    english: "Bat Yam - Yoseftal",
    russian: "Бат Ям - Йосеф Таль",
    arabic: "بات يام - يوسفطال",
    lat: 32.014588,
    lon: 34.762082,
    image: "bat-yam-yoseftal",
    alias: ["אלי כהן", "Eli Cohen", "Эли Коэн", "إلي كوهين"],
  },
  {
    id: "4690",
    hebrew: "בת ים - קוממיות",
    english: "Bat Yam - Komemiyut",
    russian: "Бат Ям - Комемуют",
    arabic: "بات يام - كوميميوت",
    lat: 32.000989,
    lon: 34.759437,
    image: "bat-yam-komemiyut",
  },
  {
    id: "9800",
    hebrew: "ראשון לציון - משה דיין",
    english: "Rishon LeTsiyon - Moshe Dayan",
    russian: "Ришон-Ле-Цион станция им. Моше Даяна",
    arabic: "ريشون لتسيون -موشي ديان",
    lat: 31.987987,
    lon: 34.757392,
    image: "rishon-moshe-dayan",
  },
  {
    id: "9000",
    hebrew: "יבנה מערב",
    english: "Yavne - West",
    russian: "Явне-Запад",
    arabic: "ياڤني - غرب",
    lat: 31.891676,
    lon: 34.731526,
    image: "yavne-west",
  },
  {
    id: "9600",
    hebrew: "שדרות",
    english: "Sderot",
    russian: "Сдерот",
    arabic: "سديروت",
    lat: 31.515877,
    lon: 34.586059,
    image: "sderot",
  },
  {
    id: "9650",
    hebrew: "נתיבות",
    english: "Netivot",
    russian: "Нетивот",
    arabic: "نتيفوت",
    lat: 31.410871,
    lon: 34.571423,
    image: "netivot",
  },
  {
    id: "9700",
    hebrew: "אופקים",
    english: "Ofakim",
    russian: "Офаким",
    arabic: "أوفاكيم",
    lat: 31.322628,
    lon: 34.634078,
    image: "ofakim",
  },
  {
    id: "3310",
    hebrew: "נתניה - ספיר",
    english: "Netanya - Sapir",
    russian: "Нетания – Сапир",
    arabic: "نتانيا - سبير",
    lat: 32.278832,
    lon: 34.865175,
    image: "netanya-sapir",
  },
  {
    id: "1240",
    hebrew: "יקנעם - כפר יהושע",
    english: "Yokne'am - Kfar Yehoshu'a",
    russian: "Йокнеам – Кфар-Иегошуа",
    arabic: "يوكنعام – كفار يهوشوع",
    lat: 32.681069,
    lon: 35.124624,
    image: "yokneam",
  },
  {
    id: "1250",
    hebrew: "מגדל העמק - כפר ברוך",
    english: "Migdal Ha'emek - Kfar Barukh",
    russian: "Мигдаль-Ха-Эмек – Кфар Барух",
    arabic: "مجدال هعيمك – كفار باروخ",
    lat: 32.647554,
    lon: 35.207967,
    image: "migdal-haeemek",
  },
  {
    id: "1260",
    hebrew: "עפולה ר. איתן",
    english: "Afula R.Eitan",
    russian: "Афула Р. Эйтан",
    arabic: "العفولة  ر. ايتان",
    lat: 32.622003,
    lon: 35.294721,
    image: "afula",
  },
  {
    id: "1280",
    hebrew: "בית שאן",
    english: "Beit She'an",
    russian: "Бейт Шеан",
    arabic: "بيت شآن",
    lat: 32.514683,
    lon: 35.488106,
    image: "beit-shean",
    alias: ["דוד לוי", "David Levy", "Давид Леви", "دڤيد لِڤي"],
  },
  {
    id: "1820",
    hebrew: "אחיהוד",
    english: "Ahihud",
    russian: "Ахихуд",
    arabic: "احيهود",
    lat: 32.91159,
    lon: 35.174068,
    image: "ahihud",
  },
  {
    id: "1840",
    hebrew: "כרמיאל",
    english: "Karmiel",
    russian: "Кармиэль",
    arabic: "كرميئيل",
    lat: 32.924764,
    lon: 35.294801,
    image: "karmiel",
  },
  {
    id: "2940",
    hebrew: "רעננה מערב",
    english: "Ra'anana West",
    russian: "Раанана-Вест",
    arabic: "رعنانا – غرب",
    lat: 32.180006,
    lon: 34.850737,
    image: "raanana-west",
  },
  {
    id: "2960",
    hebrew: "רעננה דרום",
    english: "Ra'anana South",
    russian: "Раанана Южный",
    arabic: "رعنانا الجنوبية",
    lat: 32.172568,
    lon: 34.886294,
    image: "raanana-south",
  },
  {
    id: "6150",
    hebrew: "קריית מלאכי - יואב",
    english: "Kiryat Malakhi – Yoav",
    russian: "Кирьят Малахи-Йоав",
    arabic: "كريات ملاخي – يوآڤ",
    lat: 31.747335,
    lon: 34.822479,
    image: "kiryat-malachi",
  },
  {
    id: "680",
    hebrew: "ירושלים - יצחק נבון",
    english: "Jerusalem - Yitzhak Navon",
    russian: "Иерусалим - Ицхак Навон",
    arabic: "أورشليم – يتسحاق ناڤون",
    lat: 31.788062,
    lon: 35.202545,
    image: "jerusalem-itzhak-navon",
    blurhash: "9KGSDi?w",
  },
  {
    id: "6900",
    hebrew: "מזכרת בתיה",
    english: "Mazkeret Batya",
    russian: "Мазкерет Батья",
    arabic: "مزكيرت باتيا",
    lat: 31.841507,
    lon: 34.855948,
    image: "mazkeret-batya",
  },
  {
    id: "3900",
    hebrew: "חדרה - מזרח",
    english: "Hadera - East",
    russian: "Хадера – Восток",
    arabic: "حديرا - شرق",
    lat: 32.450496,
    lon: 35.020744,
    image: "hadera-east",
  },
  {
    id: "4300",
    hebrew: "שומרון - טייבה",
    english: "Shomron – Tayyiba",
    russian: "Шомрон – Тайбе",
    arabic: "هشومرون – الطيبة",
    lat: 32.277165,
    lon: 35.005913,
    image: "shomron-taybe",
  },
  {
    id: "4310",
    hebrew: "טירה - כוכב יאיר",
    english: "Tira - Kokhav Ya'ir",
    russian: "Тира – Кохав-Яир",
    arabic: "الطيرة - كوخاڤ يَئير",
    lat: 32.234875,
    lon: 34.957647,
    image: "tira-kokhav-yair",
  },
]

export const stationsById: Record<string, Station> = Object.fromEntries(stations.map((station) => [station.id, station]))
