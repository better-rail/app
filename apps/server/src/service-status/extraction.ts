...
- "החל מהלילה שבין רביעי לחמישי בשעה 00:01" starts Thursday 00:01. "עד יום ראשון בשעה 04:00 עם חידוש השירות" ends Sunday 04:00. "ביום שישי בלבד" runs Friday 00:00 to Saturday 04:00. "מוצ"ש" starts Saturday 18:00. "החל מהשעה 22:00 בקירוב" starts at 22:00 that day. When an end is given only as a day, use 04:00 of the following day (traffic returns to normal at 04:00). When works "return to normal on Sunday at 04:00", every window of that weekend ends then — but only then: "השירות יחזור לסדרו במוצאי שבת" ends Saturday 18:00, because Saturday-night trains run; "יחזור לסדרו ביום שישי בשעה 12:00" ends Friday 12:00. The end the update gives always wins over the 04:00 default.
...
- reason: a label of a few words (at most five or so in each language) on why, as the update gives it, without dates or station names: "עבודות תשתית", "שריפה בקרבת המסילה", "תקלה בתשתית החשמל". Keep the name the update gives the works — a project, a line, a place — because passengers know them by it, but as a label, not a sentence: "עבודות מסילת 431" / "Line 431 works", "חשמול קו העמק" / "Valley line electrification", not "חיבור מסילת 431 לרשת המסילות הארצית". Drop Israel Railways' stock qualifiers, which add no information: "עבודות תשתית מצילות חיים החיוניות לבטיחות הנסיעה" is just "עבודות תשתית" / "Infrastructure works". Give the same meaning in Hebrew, English, Russian and Arabic.
- Every translation is entirely in its own script: transliterate station and place names ("Modi'in Center", "Модиин-Центр", "موديعين مركز", "كريات جات"), never leave a Hebrew word inside the English, Russian or Arabic text.
