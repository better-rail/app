...
describe("planTravels", () => {
  it("finds direct trains and orders them by departure", () => {
    ...
    expect(travels[0].trains[0].routeStations.map((s: { arrivalTime: string }) => s.arrivalTime)).toEqual([
      "08:00",
      "08:20",
      "08:35",
    ])
  ...
  it("uses the 5-min minimum, skipping an earlier 4-min option when a comfortable change exists", () => {
    const trips = table(
      ...
      trip("tight", 20, [
        [2300, "08:34", 2],
        [999, "09:00", 1],
      ]), // 4-min change, arrives 09:00
      trip("comfy", 21, [
        [2300, "08:50", 2],
        [999, "09:20", 1],
      ]), // 20-min change, arrives 09:20
    )
  ...
  it("rejects a transfer that requires waiting more than an hour", () => {
    ...
    expect(planTravels(ok, 3700, 1300, ts("07:00"))[0].trains.map((t: { trainNumber: number }) => t.trainNumber)).toEqual([
      200, 202,
    ])
  })
  ...
  it("bounds results to the requested day via endTs", () => {
    const trips = table(
      trip("a", 10, [
        [3700, "23:30", 1],
        [3400, "23:55", 1],
      ]), // departs within the day
      trip("b", 11, [
        [3700, "24:30", 1],
        [3400, "24:55", 1],
      ]), // 00:30 next day -> excluded
    )
  ...
  it("hides a departure a later one overtakes, but only while that later one runs", () => {
    ...
    const leg = (id: string, n: number, dep: string, mid: string) =>
      trip(id, n, [
        [3700, dep, 1],
        [3500, mid, 2],
      ])
    const onward = (id: string, n: number, dep: string, arr: string) =>
      trip(id, n, [
        [3500, dep, 3],
        [3400, arr, 1],
      ])
    const trips = table(
      leg("a1", 801, "08:08", "08:20"),
      onward("a2", 802, "08:26", "08:40"),
      leg("b1", 811, "08:15", "08:19"),
      onward("b2", 812, "08:24", "08:30"),
      leg("c1", 821, "08:28", "08:40"),
      onward("c2", 822, "08:46", "09:00"),
      leg("d1", 1001, "10:08", "10:20"),
      onward("d2", 1002, "10:26", "10:40"),
      leg("e1", 1101, "11:08", "11:20"),
      onward("e2", 1102, "11:26", "11:40"),
    )
    const travels = planTravels(trips, 3700, 3400, ts("07:00"))
  ...
  it("keeps a slow route when the faster departure is over an hour of waiting away", () => {
    const trips = table(
      ...
      trip("fast", 101, [
        [3700, "09:15", 1],
        [3400, "10:10", 1],
      ]), // arrives 10 min later, 75 min of waiting
    )
  ...
  it("offers a change off a direct train when it gets there sooner, keeping the direct too", () => {
    const trips = table(
      ...
      trip("b", 302, [
        [2300, "08:30", 5],
        [1300, "08:45", 1],
      ]), // …changing at 2300 arrives 08:45
    )
    const travels = planTravels(trips, 3700, 1300, ts("07:00"))
  ...
  it("keeps a change that is genuinely on the way", () => {
    ...
    const trips = table(
      ...
      trip("direct", 640, [
        [9650, "10:47", 1],
        [3500, "12:38", 2],
      ]), // 3 min sooner, inside the margin
    )
  ...
  it("caps changes at one more than the quickest way to make the trip", () => {
    ...
    const trips = table(
      ...
      trip("twoC", 302, [
        [3600, "08:40", 3],
        [3400, "09:32", 1],
      ]), // …arrives 09:32, last of the three
    )
    const travels = planTravels(trips, 3700, 3400, ts("07:00"))
  ...
  describe("realtime (SIRI) injection", () => {
    ...
    it("marks skipped stops and passes train-level cancellation + live last stop", () => {
      ...
      expect(cancelled.routeStations.map((s: { cancelled?: boolean }) => s.cancelled)).toEqual([undefined, true, undefined])
